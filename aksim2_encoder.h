...
class Aksim2Encoder : public EncoderBase {
 public:
  ...
  int32_t read() override {
    ...
    // 17 bits of nothing, then number of data bits 18-20, then error bit, warn
    // bit, and crc6 17+20+2+6 = 45 -> 6 bytes raw value will be 31 bits with a
    // leading zero
    raw_value_ =
        data_in_[2] << 24 | data_in_[3] << 16 | data_in_[4] << 8 | data_in_[5];
    Diag diag;
    diag.word = (raw_value_ >> (31 - 8 - nbits_)) & 0xFF;
    diag_.err = diag.err;
    diag_.warn = diag.warn;
    crc_calc_ = ~CRC_BiSS_43_24bit(raw_value_ >> (31 - 2 - nbits_)) &
                0x3F;  // crc of data plus 2 status bits
    ...
    if (diag_.crc6 && diag_.err) {
      ...
      int32_t diff =
          (int32_t)(shift_value - last_shift_value_) >> (32 - nbits_);
  ...
  bool index_received() override { return true; }
  DiagProcessed diag_ = {};
  uint8_t crc_calc_ = {};
  Diag diag_raw_ = {};
