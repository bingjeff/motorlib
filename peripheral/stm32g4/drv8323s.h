...
class DRV8323S : public DriverBase {
 public:
  DRV8323S(SPI_TypeDef &regs, Logger &logger,
  ...
  void enable() override {
    ...
    for (uint8_t i = 0; i < sizeof(param->drv_regs) / sizeof(uint16_t); i++) {
      ...
      logger_.log_printf("address: %d, reg_out: %x, reg_in: %x", address,
                         reg_out, reg_in);
    }
    ...
    DriverBase::enable();
    if (!drv_regs_error) {
      logger_.log("drv8323s configure success");
    } else {
      logger_.log_printf("drv8323s configure error: %02x", drv_regs_error);
    }
  }
