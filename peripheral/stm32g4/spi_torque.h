...
class SPITorque final : public TorqueSensorBase {
 public:
  SPITorque(SPI_TypeDef &regs, GPIO &gpio_cs, DMA_Channel_TypeDef &tx_dma,
            DMA_Channel_TypeDef &rx_dma, uint8_t decimation = 50,
  ...
  void reinit() override {
    if (!*register_operation_) {
      regs_.CR2 = (7 << SPI_CR2_DS_Pos) | SPI_CR2_FRXTH | SPI_CR2_RXDMAEN |
                  SPI_CR2_TXDMAEN;  // 8 bit
      regs_.CR1 = SPI_CR1_MSTR | (3 << SPI_CR1_BR_Pos) | SPI_CR1_CPHA |
                  SPI_CR1_SSI | SPI_CR1_SSM |
                  SPI_CR1_SPE;  // baud = clock/16 spi mode 1
    }
  }
  ...
  void trigger() override {
    ...
    if (!*register_operation_) {
      ...
      tx_dma_.CCR = DMA_CCR_EN | DMA_CCR_MINC |
                    DMA_CCR_DIR;  // DIR = 1 > read from memory
    }
  }
  ...
  float read() override {
    if (!*register_operation_) {
      if (count_ == 0) {
        ...
        gpio_cs_.set();
        // process result
        result0_ = (uint32_t)data_in_[4] << 24 | (uint32_t)data_in_[3] << 16 |
                   (uint16_t)data_in_[2] << 8 | data_in_[1];
        result1_ = (uint32_t)data_in_[8] << 24 | (uint32_t)data_in_[7] << 16 |
                   (uint16_t)data_in_[6] << 8 | data_in_[5];
