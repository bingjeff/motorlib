...
MainLoop::MainLoop(FastLoop &fast_loop, PositionController &position_controller,
                   ...
                   DriverBase &driver, Logger &logger, RoundRobinLogger &round_robin_logger,const MainLoopParam &param,
                   HardwareBrakeBase &brake)
    : param_(param),
      ...
      driver_(driver),
      logger_(logger),
      round_robin_logger_(round_robin_logger),
      brake_(brake) {
  set_param(param);
...
void MainLoop::update() {
  ...
  status_.output_position =
      output_encoder_rad + param_.output_encoder.bias +
  ...
  if (((status_.motor_position > param_.encoder_limits.motor_controlled_max &&
        iq_des >= 0) ||
       (status_.motor_position < param_.encoder_limits.motor_controlled_min &&
        iq_des <= 0)) &&
      started_) {
  ...
  if (communication_.send_acknowledged()) {
    round_robin_logger_.get_next_data(&status_.rr_data);
  }
...
void MainLoop::set_param(const MainLoopParam &param) {
...
void MainLoop::set_rollover(float rollover) {
...
void MainLoop::adjust_output_encoder(float adjustment) {
  param_.output_encoder.bias += adjustment;
}
void MainLoop::set_motor_encoder_bias(float bias) {
  motor_encoder_bias_ = bias;
}
const MainLoopStatus &MainLoop::get_status() const {
  return status_stack_.top();
}
void MainLoop::set_started() { started_ = true; }
void MainLoop::set_mode(MainControlMode mode) {
  if (mode != mode_ || safe_mode_ != last_safe_mode_) {
    ...
    if (safe_mode_) {
      ...
      if (param_.safe_mode_driver_disable && !(mode == DRIVER_ENABLE)) {
        logger_.log_printf("safe mode driver disable, mode: %d", mode);
        driver_.disable();
      }
    } else {
      led_.set_rate(1);
  ...
  status_.mode = mode;
  receive_data_.mode_desired = mode;  // TODO: Determine what this mode does.
}
bool MainLoop::driver_enable_triggered() {
...
bool MainLoop::driver_disable_triggered() {
...
void MainLoop::set_command(const MotorCommand &command) {
  internal_command_ = command;
  internal_command_received_ = true;
}
bool MainLoop::is_started() const { return started_; }
bool MainLoop::first_command_received() const {
  return first_command_received_;
}