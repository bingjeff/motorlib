...
void system_run() { System::run(); }

void main_loop_interrupt() { System::main_loop_interrupt(); }

void fast_loop_interrupt() { System::fast_loop_interrupt(); }

void system_log(std::string s) { System::log(s); }
...
void System::run() {
  ...
  api.add_api_variable("mode",
                       new APIUint32((uint32_t *)&actuator_.main_loop_.mode_));
  api.add_api_variable(
      "kp",
      new APIFloat(&actuator_.main_loop_.position_controller_.controller_.kp_));
  api.add_api_variable(
      "kd",
      new APIFloat(&actuator_.main_loop_.position_controller_.controller_.kd_));
  api.add_api_variable(
      "ki",
      new APIFloat(&actuator_.main_loop_.position_controller_.controller_.ki_));
  api.add_api_variable(
      "ki_limit",
      new APIFloat(
  ...
  api.add_api_variable(
      "max",
      new APIFloat(
          &actuator_.main_loop_.position_controller_.controller_.command_max_));
  api.add_api_variable(
      "vlimit",
      new APIFloat(&actuator_.main_loop_.position_controller_.velocity_limit_));
  ...
  api.add_api_variable(
      "vkp",
      new APIFloat(&actuator_.main_loop_.velocity_controller_.controller_.kp_));
  api.add_api_variable(
      "vki",
      new APIFloat(&actuator_.main_loop_.velocity_controller_.controller_.ki_));
  api.add_api_variable(
      "vki_limit",
      new APIFloat(
  ...
  api.add_api_variable(
      "vmax",
      new APIFloat(
          &actuator_.main_loop_.velocity_controller_.controller_.command_max_));
  api.add_api_variable(
      "vacceleration_limit",
      new APIFloat(
  ...
  api.add_api_variable(
      "iki_limit", new APIFloat(&actuator_.fast_loop_.foc_->pi_iq_->ki_limit_));
  api.add_api_variable(
  ...
  api.add_api_variable("messages_version",
                       new APICallback([]() { return MOTOR_MESSAGES_VERSION; },
                                       [](std::string s) {}));
  api.add_api_variable(
      "usb_err",
      new APIUint32(&((USBCommunication *)&communication_)->usb_.error_count_));
  ...
  api.add_api_variable(
      "state_command_max",
      new APIFloat(&actuator_.main_loop_.state_controller_.param_.command_max));
  api.add_api_variable(
      "state_ff_tau",
  ...
  API_ADD_FILTER(state_velocity_error_filter, FirstOrderLowPassFilter,
  ...
  api.add_api_variable("power_avg",
                       new const APIFloat(&actuator_.main_loop_.status_.power));
  api.add_api_variable(
      "energy",
...
void System::log(std::string str) { logger.log(str); }

std::string System::get_log() { return logger.get_log(); }
