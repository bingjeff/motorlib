...
void system_run() { System::get_instance().run(); }

void main_loop_interrupt() { System::get_instance().main_loop_interrupt(); }

void fast_loop_interrupt() { System::get_instance().fast_loop_interrupt(); }
...
System::System(std::unique_ptr<Actuator> actuator,
    ...
    : actuator_{std::move(actuator)},
      ...
      round_robin_logger_{std::move(round_robin_logger)},
      api_{},
      count_{0} {}

System& System::init(std::unique_ptr<Actuator>& actuator,
                     std::unique_ptr<CommunicationBase>& communication,
                     std::unique_ptr<Logger>& logger,
                     std::unique_ptr<RoundRobinLogger>& round_robin_logger) {
  if (singleton_ == nullptr) {
    singleton_ = new System(std::move(actuator), std::move(communication),
                            std::move(logger), std::move(round_robin_logger));
  }
  return *singleton_;
}
System& System::get_instance() { return *singleton_; }
...
#define API_ADD_FILTER(name, type, location)                             \
  ...
  api_.add_api_variable(                                                 \
      #name, new APICallbackFloat(get_filt_##name, set_filt_##name))
...
void System::run() {
  ...
  api_.add_api_variable("system_count", new APIUint32((uint32_t*)&count_));
  api_.add_api_variable("mode",
                        new APIUint32((uint32_t*)&actuator_->main_loop_.mode_));
  api_.add_api_variable(
      "kp", new APIFloat(
  ...
  api_.add_api_variable("cpu_frequency", new APIUint32(&cpu_frequency));
  api_.add_api_variable("t_exec_fastloop", new APIUint32(&t_exec_fastloop));
  api_.add_api_variable("t_exec_mainloop", new APIUint32(&t_exec_mainloop));
  api_.add_api_variable("t_period_fastloop", new APIUint32(&t_period_fastloop));
  api_.add_api_variable("t_period_mainloop", new APIUint32(&t_period_mainloop));
  api_.add_api_variable(
  ...
  api_.add_api_variable("ikp",
                        new APIFloat(&actuator_->fast_loop_.foc_->pi_iq_->kp_));
  api_.add_api_variable("iki",
  ...
  api_.add_api_variable("idkp",
                        new APIFloat(&actuator_->fast_loop_.foc_->pi_id_->kp_));
  api_.add_api_variable("idki",
  ...
  api_.add_api_variable("log", new APICallback([&]() { return get_log(); },
                                               [&](std::string s) { log(s); }));
  api_.add_api_variable("messages_version",
  ...
  api_.add_api_variable(
      "mcpr",
      new const APIUint32(&actuator_->fast_loop_.param_.motor_encoder.cpr));
  api_.add_api_variable(
      "ocpr",
      new const APIFloat(&actuator_->main_loop_.param_.output_encoder.cpr));
  api_.add_api_variable("irange", new const APICallbackFloat([&]() {
                          return 2048 * actuator_->fast_loop_.param_.adc1_gain;
                        }));
  api_.add_api_variable("stack_free",
                        new const APICallbackUint32(get_stack_free));
  api_.add_api_variable("stack_used",
                        new const APICallbackUint32(get_stack_used));
  api_.add_api_variable("heap_free",
                        new const APICallbackUint32(get_heap_free));
  api_.add_api_variable("heap_used",
  ...
  api_.add_api_variable(
      "fast_log", new const APICallback([&]() {
        ...
        for (int i = 0; i < 100; i++) {
          ...
          logger_->log_printf(
              "%d, %f, %f, %f, %f, %f, %f, %f, %f", status.timestamp,
              status.foc_command.measured.motor_encoder,
              status.foc_command.measured.i_a, status.foc_command.measured.i_b,
  ...
  api_.add_api_variable("disable_safe_mode", new const APICallback([&]() {
                          actuator_->main_loop_.param_.error_mask.all =
                              ERROR_MASK_NONE;
                          return "ok";
                        }));
  api_.add_api_variable(
      "error_mask", new const APICallback([&]() {
        return u32_to_hex(actuator_->main_loop_.param_.error_mask.all);
      }));
  api_.add_api_variable("help", new const APICallback([&]() {
                          return api_.get_all_api_variables();
                        }));
  api_.add_api_variable("api_length", new const APICallbackUint16([&]() {
                          return api_.get_api_length();
                        }));
  ...
  while (1) {
    ...
    if (s[0] != 0) {
      auto response = api_.parse_string(s);
...
#undef API_ADD_FILTER  // Clean-up helper macro.

void System::main_loop_interrupt() { actuator_->main_loop_.update(); }

void System::fast_loop_interrupt() { actuator_->fast_loop_.update(); }

void System::log(std::string str) { logger_->log(str); }

std::string System::get_log() { return logger_->get_log(); }
