...
// === Implementation ===
ADS1235::ADS1235(SPIDMA &spidma, PGAGain pga_gain, SPS sps,
                 volatile int *register_operation)
...
float ADS1235::read() {
  if (!*register_operation_) {
    ...
    int32_t torque_int =
        signextend<int32_t, 24>((data_[2]) << 16 | (data_[3] << 8) | data_[4]);
    torque_ = gain_ * torque_int + bias_;
  }
  return torque_;
}

uint8_t ADS1235::read_register(uint8_t address) {
  (*register_operation_)++;
...
bool ADS1235::set_register(uint8_t address, uint8_t value) {
  (*register_operation_)++;
  bool retval = true;
